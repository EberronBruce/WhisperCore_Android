...
import com.whispercpp.whisper.media.DefaultAudioDecoder
...
class Whisper(applicationContext: Context,     // Allow injecting controller for testing
	...
	private val controller: WhisperState = controllerOverride ?: run {
		// This block executes only if controllerOverride is null
		Log.d(TAG, "Whisper API initializing default controller.")
		val audioDecoder: AudioDecoder = DefaultAudioDecoder(appContext)
		WhisperState(appContext, audioDecoder)
	}

	// Scope for API methods that launch coroutines and ensure callbacks on Main if needed
	val apiScope = CoroutineScope(Dispatchers.Main + SupervisorJob())
	...
	init {
		if (controllerOverride != null) {
			Log.d(TAG, "Whisper API initialized with a testing controller.")
		} else {
			Log.d(TAG, "Whisper API initialized with a default controller.")
		}
	}
	 ...
	 * (Corresponds to `isRecording()` in Swift)
	 * @return `true` if recording, `false` otherwise.
	 */
	fun isRecording(): Boolean = controller.isRecording

	/**
	 * Indicates whether Whisper is ready to perform transcription.
	 * (Corresponds to `canTranscribe()` in Swift)
	 * @return `true` if ready to transcribe, `false` otherwise.
	 */
	fun canTranscribe(): Boolean = controller.canTranscribe
	...
	fun callRequestRecordPermission() { // Matches your iOS API name
		...
		if (currentlyGranted) {
			...
			// Optionally, you could notify the delegate that permission is confirmed,
			// though onRecordPermissionResult is the main path for grant notification.
			// delegate?.permissionOutcome(true, "Already granted")
		} else {
